...
- **统计卡片**：爱车相伴天数、总里程（表显，Node-RED 连接时取实时值）、预计续航（由 Node-RED 集成侧按当前电量与历史平均电耗统一计算，app 直接显示）、累计充电度数、平均电耗（墙端）、每公里成本。
...
应用核心功能**完全离线可用**，不依赖任何服务器：

- 手动记账（充电/费用）、统计、图表、电池健康、多车辆、导入导出、本地每日快照与恢复，全部在浏览器本地完成；
- PWA 离线缓存：首次访问后断网也能打开使用；
- Node-RED / Home Assistant 只是**可选增强**：自动记录充电、打开自动同步、实时续航、备份到 NAS。未配置或连不上时，这些功能会安静跳过/显示「—」，不影响手动使用和本地备份。
