...
- **统计卡片**：爱车相伴天数（始终显示，未设置时显示 —，可在设置里填日期）、总里程（表显，最新一条记录）、累计充电度数、平均车端/墙端电耗、每公里成本（两行三列排布；月度总支出在费用页看）。
...
- **Node-RED 自动记录同步**：配合 Home Assistant + Node-RED 自动记录插座充电（换电跳变检测、电量表差值算度数），app 内可填接口地址、一键同步记录、检查连接状态。
- 当前车辆的名称、**爱车相伴日期**（记录页显示相伴天数）、电池容量、充电效率、默认电价、电耗单位。
